...
this tokenizer is generated by flex from  [/src/grammar/for90.l](/src/grammar/for90.l)
...
## parser
parser is generated by bison from [/src/grammar/for90.y](/src/grammar/for90.y)
calls `int yylex(void)` to get one token from tokenizer at a time, which is `pure_yylex` in flex, and `simple_yylex` in simple\_lexer
...
2. Step 2:
    ...
    in `regen_all_variables`, includes a do-loop in which `regen_vardef` is called to every variable. A more 1-pass for-loop is not suitable here, because `regen_vardef` may introduce new variables, according to Step 1

    enumerate all `VariableInfo` of this suite several times UNTIL ALL VARIABLES ARE GENERATED(`.generate` = true)

3. Step 3:

    in `regen_all_variables_decl_str`, according to function's 2-phase generating strategy, `regen_all_variables_decl_str` is called in `regen_function_2`
