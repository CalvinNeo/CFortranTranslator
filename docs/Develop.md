...
1. Step 1:
    1. when encounter a `UnknownVariant`(in `regen_exp`, when the AST is building):
        ...
        all variables, **once** reached by the parser and not registered to symbol table `gen_context().variables` yes, will be added to symbol table, which includes: 
    ...
    3. when encounter `NT_VARIABLEDEFINESET` and `NT_VARIABLEDEFINE`(in `regen_stmt`, after the AST is built):
        ...
        these two nodes are generated in [/src/grammar/for90.y](/src/grammar/for90.y), generate `NT_VARIABLEDEFINESET` and `NT_VARIABLEDEFINE`
